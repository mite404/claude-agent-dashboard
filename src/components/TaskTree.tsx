        ...
        <div key={node.id}>
          ...
          {node.children.length > 0 && (
            <div className="relative mt-2 pl-6">
              {/* Vertical connector */}
              <div className="absolute top-0 bottom-2 left-3 w-px bg-white/10" aria-hidden="true" />
              <TaskTree nodes={node.children} depth={depth + 1} onStatusChange={onStatusChange} />
            </div>
          )}
        </div>
      ))}
