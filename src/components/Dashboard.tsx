...
export default function Dashboard() {
  ...
  const handleStatusChange = useCallback(
    (_taskId: string, _newStatus: TaskStatus) => {
      // Optimistically refresh after a short delay for the server to settle
  ...
  const defaultSessionId = tasks.find((t) => t.sessionId)?.sessionId ?? 'kanban-pool';
  ...
  return (
    <div className="min-h-screen bg-stone-950 px-4 py-6 md:px-8">
      ...
      {viewMode === 'board' ? (
        <KanbanBoard tasks={tasks} sessionId={defaultSessionId} onRefresh={refresh} />
      ) : (
        <TaskTable
          tree={tree}
